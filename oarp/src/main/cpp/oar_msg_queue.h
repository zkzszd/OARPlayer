...
#ifndef OAR_MSG_QUEUE_H
#define OAR_MSG_QUEUE_H
...
inline static int msg_queue_put(MessageQueue *q, AVMessage *msg)
{
    ...
    OAR_LockMutex(q->mutex);
    ret = msg_queue_put_private(q, msg);
    OAR_UnlockMutex(q->mutex);
...
inline static void msg_queue_init(MessageQueue *q)
{
    ...
    q->mutex = OAR_CreateMutex();
    q->cond = OAR_CreateCond();
    q->abort_request = 1;
}
...
inline static void msg_queue_destroy(MessageQueue *q)
{
    ...
    OAR_DestroyMutex(q->mutex);
    OAR_DestroyCond(q->cond);
}
